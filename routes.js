...
async function routes(fastify, options) {
    ...
    fastify.post('/', {schema: addTodo}, async function(request, reply) {
        ...
        const query = {
            text: `INSERT INTO todo (id, name, department, done)
    ...
    fastify.delete('/:id', {schema: deleteTodo}, async function(request, reply) {
        ...
        try {
            const {rows} = await client.query('DELETE FROM todo WHERE id = $1 RETURNING *', [request.params.id])
            console.log(rows[0])
            reply.code(204)
        } catch(err) {
            throw new Error(err)
